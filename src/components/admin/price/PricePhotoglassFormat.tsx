...
import { PrisePhotoglassFormat } from "../../../interfaces/Price";

const PricePhotoglassFormat = () => {
  const context = useContext(PriseContext);

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingData, setEditingData] = useState<PrisePhotoglassFormat[] | null>(null);
  const [form] = Form.useForm();

  useEffect(() => {
    if (context && context.isPriceLoaded) {
      setEditingData(context.prisePhotoglassFormat || []); // Загружаем данные из контекста
    }
  }, [context]);
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      if (context?.updatePrice) {
        context.updatePrice("prisePhotoglassFormat", values.items); // Передаём массив
      }
    });
  };
  ...
  return (
    <div className="container">
      <Table
        ...
        columns={[
          {
            title: "Формати фотоскла | Розміри",
            dataIndex: "size",
            key: "size",
            width: "60%",
      ...
      <Modal
        title="Редагування форматів фотоскла"
        open={isModalVisible}
...
export default PricePhotoglassFormat;
