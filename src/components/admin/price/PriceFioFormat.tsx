...
const PriceFioFormat = () => {
  const context = useContext(PriseContext);

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingData, setEditingData] = useState<PriseFioFormat[] | null>(null);
  const [form] = Form.useForm();

  useEffect(() => {
    if (context && context.isPriceLoaded) {
      setEditingData(context.priseFioFormat || []); // Загружаем данные из контекста
    }
  }, [context]);
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      if (context?.updatePrice) {
        context.updatePrice("priseFioFormat", values.items); // Передаём массив
      }
    });
  };
    ...
    <div className="container">
      <Table
        ...
        columns={[
          {
            title:
              "Піскоструй: Прізвище, ім'я, по батькові | Стандартні формати",
            dataIndex: "size",
            key: "size",
            width: "60%",
