...
import { PriseCross } from "../../../interfaces/Price";

const PriceCross = () => {
  const context = useContext(PriseContext);

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingData, setEditingData] = useState<PriseCross[] | null>(null);
  const [form] = Form.useForm();

  useEffect(() => {
    if (context && context.isPriceLoaded) {
      setEditingData(context.priseCross || []); // Загружаем данные из контекста
    }
  }, [context]);
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      if (context?.updatePrice) {
        context.updatePrice("priseCross", values.items); // Передаём массив
      }
    });
  };
  ...
  return (
    <div className="container">
      <Table
        ...
        columns={[
          {
            title: "Хрестик латунний | розміри",
            dataIndex: "size",
            key: "size",
            width: "60%",
